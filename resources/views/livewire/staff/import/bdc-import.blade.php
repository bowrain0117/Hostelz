<div class="my-5">
    ...
    @if($currentBatch)
        <div class="col-md-12">
            <p><strong>job name: </strong>: {{ $currentBatch->name }}</p>
            <p><strong>total Jobs: </strong>: {{ $currentBatch->totalJobs }}</p>
            <p><strong>pending Jobs: </strong>: {{ $currentBatch->pendingJobs }}</p>

            <x-progress-bar :percentage="$percentageCurrentBatch"/>
        </div>
    @endif

    @if($imports)
        <div class="col-md-12 mb-3">
            <button class="btn btn-primary" type="button" data-toggle="collapse" data-target="#importsHc"
                    aria-expanded="false" aria-controls="importsHc">
                Last 20 imports
            </button>
            <div class="collapse" id="importsHc">
