...
@section('nextToForm')
    ...
    @if ($formHandler->mode == 'updateForm' || $formHandler->mode == 'update' || $formHandler->mode == 'display')
        ...
        @if ($listing->cityInfo && $listing->cityInfo->hostelCount >= min(CityInfo::SPECIAL_HOSTELS_PAGE_MIN_HOSTELS))
            <div class="list-group">
                <a href="#" class="list-group-item active">SEO</a>
                @if ($listing->cityInfo->hostelCount >= CityInfo::SPECIAL_HOSTELS_PAGE_MIN_HOSTELS['cheap'])
                    <a href="{!! routeURL('staff-listingSpecialText', 'edit-or-create')."?listingID=$listing->id&type=" . CategorySlp::Cheap->value !!}"
                       class="list-group-item">
                        <span class="pull-right">&raquo;</span>Cheap Hostels Text
                    </a>
                @endif

                @if ($listing->cityInfo->hostelCount >= CityInfo::SPECIAL_HOSTELS_PAGE_MIN_HOSTELS['best'])
                    <a href="{!! routeURL('staff-listingSpecialText', 'edit-or-create')."?listingID={$listing->id}&type=" . CategorySlp::Best->value !!}"
                       class="list-group-item">
                        <span class="pull-right">&raquo;</span>Best Hostels Text
                    </a>
                @endif

                @if ($listing->cityInfo->hostelCount >= CityInfo::SPECIAL_HOSTELS_PAGE_MIN_HOSTELS['private'])
                    <a href="{!! routeURL('staff-listingSpecialText', 'edit-or-create')."?listingID=$listing->id&type=" . CategorySlp::Private->value !!}"
                       class="list-group-item">
                        <span class="pull-right">&raquo;</span>Private Hostels Text
                    </a>
                @endif

                @if ($listing->cityInfo->hostelCount >= CityInfo::SPECIAL_HOSTELS_PAGE_MIN_HOSTELS['party'])
                    <a href="{!! routeURL('staff-listingSpecialText', 'edit-or-create')."?listingID=$listing->id&type=" . CategorySlp::Party->value !!}"
                       class="list-group-item">
                        <span class="pull-right">&raquo;</span>Party Hostels Text
                    </a>
                @endif
            </div>
        @endif
    ...
    @elseif ($formHandler->mode == 'searchAndList' || $formHandler->mode == 'list' )
        ...
        <div class="text-muted small">
            @foreach ($formHandler->list as $rowKey => $row)
                {!! $row->id . ($row->id != $formHandler->list->last()->id ? ',' : '') !!}
            @endforeach
        </div>
