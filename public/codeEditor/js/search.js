...
function replace_next() {
  if (current_range) {
    ...
    console.log("replace("+input+", "+current_replace+")");
    if (replacement !== null) {
...
function quick_searcher (e) {
  //alert(e.keyCode);
  if(e.keyCode != 13 && e.keyCode != 38 && e.keyCode != 40 && e.keyCode != 114) return; // enter, up/down arrow, F3
  var n = $('#quick_search').val();
  if (n) {
    var opts = {
      needle: n,
      backwards: e.keyCode == 38 ? true : false, // (up arrow to search backwards)
      wrap: true,
      caseSensitive: false,
      wholeWord: false,
      scope: Search.ALL,
...
function quick_filesearch (e) {
  //alert(e.keyCode);
  if(e.keyCode != 13 && e.keyCode != 38 && e.keyCode != 40 && e.keyCode != 114) return; // enter, up/down arrow, F3
  
  var n = $('#quick_filesearch').val();
  
  if (n) {
    $('#greptab a').click();
     $('#grep_results').html("Searching...");
   
   
   var filesets = $("input[id^='grepSet_']:checked").map(function () { return this.value; }).get();
   if(filesets == '') { alert('Select file groups.'); return false; }
   var options = $("input[id^='grepOption_']:checked").map(function () { return this.id.replace('grepOption_',''); }).get();
   options.push('filenameSearch'); // make sure filenameSearch is what we're doing
   
   var item = $("#grep_results_panel");
   search_panel.select(item);
   search_panel.expand(item);
   
   $('#grep_results').html("Searching...");
   
    $('#grep_results').load('?cmd=grep', 
      { 'needle': n, 'filesets': filesets, 'options': options } );
  }
}
...
$(document).ready(function () {
  ...
  /*
  $('#dir_chooser > div.browser').fileTree({ root: '', script: '?cmd=dirChooser', expandSpeed: 200, collapseSpeed: 200 }, get_file);
  */
  
  $("#quick_search").keydown(function(e) { quick_searcher(e); });
  $("#quick_filesearch").keydown(function(e) { quick_filesearch(e); });
});

/* Grep */

function do_grep() {
   var needle = $("#grep_term").val();
   ...
   $('#grep_results').load('?cmd=grep', 
      { 'needle': needle, 'filesets': filesets, 'options': options } );
